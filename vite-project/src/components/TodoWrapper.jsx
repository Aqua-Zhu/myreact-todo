...
function TodoWrapper() {
    ...
    // 因為要判定todo是否被點擊。所以要增加一個標記屬性e=>isCompleted
    ...
    // 1.使用...其餘運算子來保留原陣列內容
    // 2.再加入新的物件內容
    const addTodo = (content2) => {
        setTodos([...todos, { content: content2, id: Math.random(), isCompleted: false, isEdit: false }])
    }

    // 建立刪除todo函式，傳給Todo元件使用
    // 使用filter方法去除被刪除的todo
    const deleteTodo = (id) => {
    ...
    const toggleIsEdit = (id) => {
        setTodos(todos.map((todo) => {
            // 逐筆檢查目前的todo.id是否等於被修改id
            // yes => 1.取出todo資料 2.修改isEdit屬性質為反向
            // no => todo不變
            // 三元運算子寫法
            return todo.id === id
                ? { ...todo, isEdit:!todo.isEdit/*等於true*/ }
                : todo
            // if else寫法
            // if(todo.id===id){
